...
const TURNOS = ["05:00", "14:00", "21:00"];
...
const PROD_COLOR = {
  "Leche Cruda":       "#eeece8",
  "Leche Entera":      "#1a6fcc",
  "Leche Descremada":  "#90cce8",
  "Leche Fortificada": "#27ae60",
  "Lactosa":           "#d4b896",
  "Suero":             "#ffe000",
  "Yogurt":            "#f4a0c0",
  "Sucio (vacío)":     "#dc2626",
...
const SecStock = ({ date }) => {
  const [data, setData]               = useState({});
  const [loading, setLoading]         = useState(true);
  const [turno, setTurno]             = useState("05:00");
  ...
  useEffect(() => {
    Promise.all([
      ...
      // Inferir producto por silo desde el último ingreso con producto
      const inferred = {};
      ingresos.forEach(ing => {
        const key = SILO_STOCK_KEY[ing.destino];
        if (key && ing.producto) inferred[key] = ing.producto;
      });
      // Auto-poblar producto faltante en todos los turnos
      let updated = d;
      let changed = false;
      TURNOS.forEach(t => {
        STOCK_SILOS.forEach(silo => {
          const sd = (((updated[t] || {}).silos) || {})[silo] || {};
          if (!sd.producto && inferred[silo]) {
            updated = {
              ...updated,
              [t]: { ...(updated[t] || {}),
                silos: { ...((updated[t] || {}).silos || {}),
                  [silo]: { ...sd, producto: inferred[silo] } } }
            };
            changed = true;
          }
        });
    ...
    calcAutoLitros(date).then(setAutoLitros);
  }, [date]);
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6, marginBottom:14 }}>
        {TURNOS.map(t => {
          ...
          return (
            <button key={t} onClick={()=>setTurno(t)} style={{ ...(turno===t?btnPrimary:btnSecondary), padding:"10px 4px" }}>
              <div style={{ fontSize:16, fontWeight:700 }}>{t}</div>
              <div style={{ fontSize:10, marginTop:2, color:turno===t?"#000a":"#22c55e88" }}>
      ...
      <div style={panel}>
        <div style={secTitle}>Responsable — {turno} hs.</div>
          ...
          <div key={silo} style={{ ...card, borderColor: hasData ? C.accentDark : C.border, padding: 12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"80px 1fr", gap:12, alignItems:"center", marginBottom:10 }}>
              ...
              <div>
                ...
                <select
                  style={{ ...inp, fontSize:12, padding:"7px 10px", WebkitAppearance:"none",
