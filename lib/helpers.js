...
// Suma por silo (clave normalizada) los litros que un lote fort descuenta:
...
//
// `siloKeyMap` mapea alias/spellings al nombre canónico (ej. "100 NUEVO" → "100 N").
// Si un silo no está mapeado, se usa su nombre tal cual.
//
// Devuelve `{ [siloKey]: litros_a_descontar }`. Las claves repetidas se acumulan
// (ej. siloOrigen y un sourceSilo de adición sobre el mismo silo).
export const fortSourceDraws = (fort, siloKeyMap = {}) => {
  const draws = {};
  const baseL = parseFloat(fort?.litrosBase) || 0;
  if (fort?.siloOrigen && baseL > 0) {
    const k = siloKeyMap[fort.siloOrigen] || fort.siloOrigen;
  ...
  (fort?.adiciones || []).forEach((a) => {
    ...
    if (L <= 0) return;
    const k = siloKeyMap[a.sourceSilo] || a.sourceSilo;
    draws[k] = (draws[k] || 0) + L;
  });
  return draws;
};
